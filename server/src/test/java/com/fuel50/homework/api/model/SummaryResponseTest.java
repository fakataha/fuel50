...
class SummaryResponseTest {
  ...
  void testBuild() {
    List<String> moodMessages =
        Arrays.asList("This is test 1.", "Another test message", "One more!");
  ...
  void testDefaults() {
    SummaryResponse result = SummaryResponse.builder().build();
  ...
  void testReference() {
    ...
    SummaryResponse result = SummaryResponse.builder().setMoodMessages(moodMessages).build();

    Assertions.assertThat(result.getMoodMessages()).isNotNull().isNotEmpty().hasSize(3);
